...
Constraint *p12Joint;
Constraint *n21_aJoint;
Constraint *n21_bJoint;
...
void init_system(void)
{
	...
	Eigen::Vector3d s1_p_sphericalJoint(0.0,0.0,0.0);
	Eigen::Vector3d s2_p_sphericalJoint(-PENDULUM_LENGTH/2.0,0.0,0.0);

	Eigen::Vector4d p_pendulum(1.0,0.0,0.0,0.0);
	Eigen::Vector3d r_pendulum(s1_p_sphericalJoint - s2_p_sphericalJoint);
	Eigen::Vector3d r_dot_pendulum(0.0,0.0,0.0);
	Eigen::Vector4d p_dot_pendulum(0.0,0.0,0.0,0.0);
	Eigen::Vector3d s1_p(1.0,0.0,0.0);
	Eigen::Vector3d s1_pa(0.0,1.0,0.0);
	Eigen::Vector3d s1_pb(0.0,0.0,1.0);
	Eigen::Vector3d s2_p(1.0,0.0,0.0);
	Eigen::Vector3d s1B_p(-PENDULUM_LENGTH/2.0,0.0,0.0);
	Eigen::Vector3d s2B_p(0.0,0.0,0.0);
	Eigen::Vector3d d(SPRING_LENGTH,0.0,0.0);
	Eigen::Vector3d r_slider(r_pendulum + s1B_p + d - s2B_p);
	Eigen::Vector4d p_slider(1.0,0.0,0.0,0.0);
	Eigen::Vector3d r_dot_slider(0.0,0.0,0.0);
	Eigen::Vector3d omega_slider_p(3.0*2.0*M_PI,0.0,0.0);
	Eigen::Vector4d p_dot_slider(0.5*caams::L(p_slider).transpose()*omega_slider_p);
...
//	sphericalJoint = new SphericalJoint(pendulumBody,datumBody,
//										s2_p_sphericalJoint,s1_p_sphericalJoint);
	p12Joint = new Parallel1_2(pendulumBody,sliderBody,s1_p,s2_p);
    n21_aJoint = new Normal2_1(pendulumBody,sliderBody,s1_pa,s1B_p,s2B_p);
    n21_bJoint = new Normal2_1(pendulumBody,sliderBody,s1_pb,s1B_p,s2B_p);
	...
	mbsystem.AddConstraint(n21_aJoint);
	mbsystem.AddConstraint(n21_bJoint);
...
int main(int argc, char **argv){
    ...
    glutCreateWindow ("Normal 2,1 test");
